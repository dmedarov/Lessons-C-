...
def test_reservation_history_audit(tmp_path: Path) -> None:
    app_module.DB_PATH = str(tmp_path / "fleet.db")
    app_module.init_db()
    ...
    with TestClient(app_module.app) as client:
        ...
        car_id = _create_car(client, admin_token, plate="CB4000AA")
        reservation_id = _create_reservation(client, car_id, employee_token, user="Ivan Petrov")
        ...
        history = client.get(f"/reservations/{reservation_id}/history")

    assert history.status_code == 200
    items = history.json()["items"]
    assert len(items) >= 2
    assert items[0]["action"] == "created"
    assert items[1]["action"] in {"approved", "rejected", "cancelled"}
