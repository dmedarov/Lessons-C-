...
def test_employee_quick_booking_surface(browser: Browser, server: str, artifact_dir: Path) -> None:
    ...
    expect(page.locator("#suggestedBookingHero")).to_be_visible(timeout=10_000)
    assert _element_is_before(page, "#suggestedBookingHero", "#reservationsDeck")
    ...
    expect(page.locator("#suggestedBookingHero")).to_be_hidden(timeout=10_000)
...
def test_approver_decision_surface(browser: Browser, server: str, artifact_dir: Path) -> None:
    ...
    expect(page.locator("#decisionRail")).to_contain_text("GSM")
    expect(page.locator("#decisionRail")).to_contain_text("Причина")
    assert _element_is_before(page, "#decisionRail", "#reservationsDeck")
    ...
    expect(page.locator("#reservationsDeck .table-wrap")).to_be_hidden()
...
def test_admin_control_surface(browser: Browser, server: str, artifact_dir: Path) -> None:
    ...
    expect(page.locator("#productionReadinessPanel")).to_be_visible()
    expect(page.locator("#productionReadinessSummary")).to_contain_text("Най-важното сега")
    pass_group = page.locator(".readiness-pass-group")
    expect(pass_group).to_be_visible()
    expect(pass_group).to_contain_text("Проверени проверки")
    pass_group.locator("summary").click()
    expect(pass_group.locator(".readiness-item--pass").first).to_be_visible()
...
def test_reception_handoff_calendar_surface(browser: Browser, server: str, artifact_dir: Path) -> None:
    ...
    assert _element_is_before(reception_page, "#receptionRail", "#calendarStudio")
    assert _element_is_before(reception_page, "#calendarStudio", "#reservationsDeck")
    ...
    expect(reception_page.locator("#receptionRail")).to_contain_text("Чака връщане")
    expect(reception_page.locator("#receptionRail")).to_contain_text("Чака предаване")
    ...
    expect(reception_page.locator("#reservationsDeck .table-wrap")).to_be_hidden()
...
def test_reception_overdue_return_next_signal(browser: Browser, server: str, artifact_dir: Path) -> None:
    ...
    first_card_text = page.locator("#receptionRail .reception-card").first.inner_text()
    assert "просрочено от" in first_card_text
