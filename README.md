...
## Роли, login и одобрение

Приложението вече има базов admin login и approval workflow.
...
- Отговорът връща bearer token, който се подава в `Authorization: Bearer <token>`.

### Правила
- Само `fleet_admin` може да добавя автомобил (`POST /cars`).
- `POST /reservations` създава резервация със статус `pending`.
- Само `fleet_admin` може да одобри/откаже:
  - `POST /reservations/{id}/approve`
  - `POST /reservations/{id}/reject`
- Отмяна (`POST /reservations/{id}/cancel`) може:
  - admin за всички,
  - employee само за собствените си резервации.

UI вече показва индикация за текуща сесия (role badge) и цветни статуси за `pending/approved/rejected/cancelled`.
Добавен е и history endpoint: `GET /reservations/{id}/history` за проследяване на действията (created/approved/rejected/cancelled).
...
- Има базови роли чрез headers (`employee`, `fleet_admin`), но няма пълна login система (JWT/OAuth).
- Има базов audit history endpoint, но без пълна enterprise audit/retention политика.
...
Подробен план за следващите фази е описан в `ROADMAP.md`.
