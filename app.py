...
def record_audit(
    ...
    conn.execute(
        """
        INSERT INTO reservation_audit(reservation_id, action, actor, note, created_at)
        VALUES(?, ?, ?, ?, ?)
        """,
        (reservation_id, action, actor, note, datetime.utcnow().isoformat() + "Z"),
    )
...
def create_reservation(payload: ReservationCreate, auth: AuthContext = Depends(get_auth_context)) -> dict:
    ...
    with get_conn() as conn:
        ...
        record_audit(conn, reservation_id, "created", auth.user, payload.purpose)
...
def approve_reservation(
    ...
    with get_conn() as conn:
        ...
        record_audit(conn, reservation_id, "approved", auth.user, payload.reason)
        row = conn.execute("SELECT * FROM reservations WHERE id=?", (reservation_id,)).fetchone()
        return dict(row)
...
def reject_reservation(
    ...
    with get_conn() as conn:
        ...
        record_audit(conn, reservation_id, "rejected", auth.user, payload.reason)
        row = conn.execute("SELECT * FROM reservations WHERE id=?", (reservation_id,)).fetchone()
        return dict(row)
...
def cancel_reservation(reservation_id: int, auth: AuthContext = Depends(get_auth_context)) -> dict:
    with get_conn() as conn:
        ...
        record_audit(conn, reservation_id, "cancelled", auth.user)
        row = conn.execute("SELECT * FROM reservations WHERE id=?", (reservation_id,)).fetchone()
        return dict(row)


@app.get("/reservations/{reservation_id}/history")
def reservation_history(reservation_id: int) -> dict[str, list[dict]]:
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM reservation_audit WHERE reservation_id=? ORDER BY id",
            (reservation_id,),
        ).fetchall()
