...
| Admin users | `templates/admin.html`, `static/app.js`, `static/styles.css`, `routers/users.py`, `schemas.py`, `e2e/test_browser_smoke.py` | pass | Destructive confirmations, role-change clarity, audit timeline readability, repeatable employee import, single-user contact correction | User deactivate confirmation is covered by `destructive-user-deactivate-confirmation.png`; role-change required-reason recovery is covered by `destructive-role-change-recovery.png`. Bulk employee import accepts `Име / Презиме / Фамилия / Телефон`, stores only `Име + Фамилия + GSM`, creates/updates `employee` users and ignores chip/tachograph columns. The user card has a compact **Контакт** action for email/GSM corrections with `contact_updated` audit, covered by `admin-contact-correction.png`. Continue with password-reset screen-reader pass. |
...
- `evidence`: latest local handoff check ran `make qa-premium` -> dependency
  audit, secret scan, Python compile, 168 pytest cases, JS syntax and 16 Playwright browser
