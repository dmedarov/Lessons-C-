...
| Accepted role-first surface blueprint | `ROADMAP.md`, `ROADMAP_IMPROVEMENTS.md`, `docs/ROLE_USER_FLOWS.md`, future `templates/*.html`, `static/app.js`, `static/styles.css` | planned | One next move per role, no table-first landing, no competing primary actions | Accepted for the next UI wave: employee suggestion-first hero, approver Decision Desk cards first, reception Handoff Desk overdue-first, admin Control Tower pulse-first. Deferred: large vanilla module split until after controlled production pilot or until the monolith creates real implementation risk. Rejected for now: framework rewrite, decorative landing visuals, heavy BI, GPS workflow expansion and AI chat. |
