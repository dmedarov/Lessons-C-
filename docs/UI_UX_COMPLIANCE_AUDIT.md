...
| Approved vs active clarity | `app.py`, `static/app.js`, `static/i18n.js`, `static/styles.css`, `templates/index.html`, `templates/admin.html`, `tests/test_app.py`, `tests/test_ui_compliance.py` | pass | Approved-but-not-started must not present as active or free, fleet/public cards reflect real operational state | Frontend now derives a dedicated `Чака вземане` presentation state for approved reservations whose start window has begun but reception has not marked `checked_out_at` yet. Current Trip Hero and lifecycle tags use that wording, fleet cards distinguish `Свободна / Чака вземане / Активен курс / Неактивна`, public calendar pills are text-backed, `/public/overview` no longer counts approved-but-not-started cars as `Свободни`, and the pre-login KPI strip now exposes `Чака вземане` as a first-class signal. |
| Notifications | `notifications_service.py`, `routers/reservations.py`, `static/app.js`, `static/i18n.js`, `static/styles.css` | pass | `aria-live`, unread badge clarity, concise copy, no noisy motion, role-correct outbound recipients | Notification lists are polite live regions and show only unread items by default; read items are hidden with a calm note. SMTP now routes to the in-app recipient's `users.email` with `SMTP_TO_EMAIL` fallback; Teams remains a shared operational webhook. Tests cover admin test email, Teams payload and request -> approver / approval -> requester + reception delivery. Manual screen-reader polling announcement still recommended before broad rollout. |
| Admin pending queue / decision rail | `templates/admin.html`, `static/app.js`, `static/i18n.js`, `static/styles.css`, `e2e/test_browser_smoke.py` | pass | Top pending decisions first, bulk selection semantics, action bar visibility, partial-failure feedback, required reject reason | Decision Rail now sits before `#reservationsDeck`, so `fleet_approver` lands on Decision Desk cards instead of filters/tables. Cards expose GSM, reason, car, time window and urgency text. The rail-level bulk button only selects pending requests and focuses the action bar; approve/reject still require the explicit bulk action, and reject requires a reason. Desktop Decision Rail is covered by `approver-desktop.png` and `admin-desktop.png`; empty reject reason recovery is covered by `destructive-reject-recovery.png` with keyboard activation, `aria-invalid` and focus-to-textarea evidence. Keyboard checkbox selection is covered by `approver-keyboard-bulk-selection.png`. |
| Admin next focus hero | `templates/admin.html`, `static/app.js`, `static/i18n.js`, `static/styles.css`, `e2e/test_browser_smoke.py` | pass | One primary action, supporting insight list, calm hierarchy, no lifecycle mutation | Control Tower now starts with a hero-style next-focus card instead of a plain summary box. The card exposes a single primary action, a status badge and a compact `InsightList` with up to three contextual signals such as overdue returns, pending approvals and readiness blockers. Browser smoke confirms the card, badge and insight list are visible on admin login. |
| Admin reception queue / reception rail | `templates/admin.html`, `static/app.js`, `static/i18n.js`, `static/styles.css`, `routers/reservations.py`, `routers/cars.py`, `e2e/test_browser_smoke.py` | pass | Overdue-first handoff desk, calendar after handoff work, role-separated start/return, direct 44 px actions, scoped pickup GPS, no dependency on current table filter | Reception Rail is now a standalone **Handoff Desk** block before the reservations module. It splits the surface into **Чака връщане** and **Чака предаване**, keeps overdue returns first, places the calendar after the handoff desk and hides the table fallback from the first reception viewport. Approval now creates a `reception_handoff` notification so reception gets **Курс чака ключове** before start. Overdue returns are prioritized as next signal for reception and admin, covered by `reception-overdue-return-signal.png` and `admin-overdue-return-signal.png`. Return confirmation keyboard/Escape recovery is covered by `destructive-return-confirmation.png`. |
| Admin users | `templates/admin.html`, `static/app.js`, `static/styles.css`, `routers/users.py`, `schemas.py`, `e2e/test_browser_smoke.py` | pass | Destructive confirmations, role-change clarity, audit timeline readability, repeatable employee import, single-user contact correction, scannable density | User deactivate confirmation is covered by `destructive-user-deactivate-confirmation.png`; role-change required-reason recovery is covered by `destructive-role-change-recovery.png`. Bulk employee import accepts `Име / Презиме / Фамилия / Телефон`, stores only `Име + Фамилия + GSM`, creates/updates `employee` users and ignores chip/tachograph columns. User cards now separate identity/signals, contact fields and action groups so contact/audit are quick actions while role/reset/activation stay secondary. The user card has a compact **Контакт** action for email/GSM corrections with `contact_updated` audit, covered by `admin-contact-correction.png`. Continue with password-reset screen-reader pass. |
...
| Production cutover | `Makefile`, `scripts/prod_check.py`, `scripts/go_live_check.py`, `scripts/smoke_live.py`, `production_readiness.py`, `routers/ops.py`, `docker-compose.postgres.yml`, `README.md`, `docs/PRODUCTION_USER_GUIDE.md` | pass | No demo seed, generated secrets, real CORS origin, matching DB URL, pinned PostgreSQL image, runtime env propagation, secret-safe admin readiness, final go-live gate | `make prod-check` validates `.env` without starting containers; compose now pins PostgreSQL major version and passes CORS, refresh TTL, rate limits, DB password and notification settings to the app container. `/health/ready` checks DB reachability; `/ops/readiness` powers the Admin UI preflight panel without returning secret values, warns when only one active admin exists, shows dedicated restore-drill freshness, renders outstanding blockers/warnings first and gives concise next-step text for each item. `make go-live-check APP_URL=...` adds restore-drill evidence, release gates and live smoke with active-admin proof. |
| Production CI gates | `.github/workflows/production-gates.yml`, `.github/workflows/tests.yml`, `Makefile`, `requirements.txt`, `requirements-dev.txt`, `Dockerfile` | needs external evidence | Python 3.12/3.14 tests, secret scan, JS syntax, production dependency audit, Docker build, current Actions versions | Local `make audit-prod` audits pinned runtime dependencies and `make release-check` runs the repeatable local gate. Both workflows now use `actions/checkout@v6` and `actions/setup-python@v6`; production checkout sets `persist-credentials: false`. CI still needs external confirmation after push. |
| Secret hygiene guard | `.gitignore`, `scripts/scan_secrets.py`, `tests/test_secret_scan.py`, `Makefile`, `.github/workflows/*.yml`, `docs/PRODUCTION_USER_GUIDE.md` | pass | No real secret values in tracked files/history, `.env.*` ignored, `.env.example` tracked, phishing/payment links treated as untrusted | `make secrets-scan` fails on real-looking `SECRET_KEY`, `POSTGRES_PASSWORD`, `DATABASE_URL`, `NETFLEET_API_KEY`, webhook/token/password assignments and private key blocks in the current checkout. `make secrets-scan-history` scans local reachable git refs/blobs for incident response. The current checkout guard runs inside `make release-check` and both GitHub workflows. A real alert still requires provider-side rotation first. |
| No silent/no noisy regression guard | `tests/test_documentation_contracts.py`, `tests/test_schema_contracts.py`, `tests/test_ui_compliance.py`, `e2e/test_browser_smoke.py`, all `.md` handoff docs | pass | Duplicate route guard, schema parity, consistent readiness score, 99/100 gate, no missing literal i18n keys, role/browser evidence | Documentation contracts now keep the 91/100 score, route/migration stats, executive QA evidence and external-evidence 99/100 gate synchronized. Treat failures here as release blockers, not documentation polish. |
| Premium QA gate | `Makefile`, `README.md`, `ROADMAP_IMPROVEMENTS.md`, `e2e/test_browser_smoke.py`, `scripts/smoke_live.py` | pass | Release-check, browser role smoke, live URL smoke, clear handoff evidence | `make qa-premium` chains release gates and browser role smoke. `make smoke-live APP_URL=...` checks `/health`, `/health/ready`, `/auth/setup-status` and `/public/overview` on a running stack. Latest local browser run passed 16 checks including responsive density, calendar/reception, operational login routing, approver reject, reception start/return and destructive recovery. |
...
| Premium visual calm | `static/styles.css`, `templates/index.html`, `templates/admin.html`, `tests/test_ui_compliance.py`, `e2e/test_browser_smoke.py` | pass | No decorative orbs, compact mobile first viewport, one useful status strip, quiet elevation | Decorative radial orb backgrounds were removed from page, hero, brand mark and dialog backdrop. Mobile KPI cards stay in a compact 3-column operational strip. Brand ownership now appears as a quiet `CNSYS` mark under FleetFlow without adding another nav item. Versioned assets were bumped to `20260422-decision-desk`; role surfaces now use Control Tower / Decision Desk / Handoff Desk / Моят курс naming. `tests/test_ui_compliance.py` blocks `radial-gradient` from returning. |
...
| Role-first login routing | `templates/index.html`, `templates/admin.html`, `static/app.js`, `e2e/test_browser_smoke.py`, `tests/test_ui_compliance.py` | pass | First screen matches the role's real job, no wrong default surface | Employees land on the employee desk and cannot keep `/admin`; `fleet_admin`, `fleet_approver` and `fleet_reception` land on `/admin` even when login starts from `/`. Browser smoke covers all three operational roles plus the employee deny path. |
...
| Accepted role-first surface blueprint | `ROADMAP.md`, `ROADMAP_IMPROVEMENTS.md`, `docs/ROLE_USER_FLOWS.md`, future `templates/*.html`, `static/app.js`, `static/styles.css` | pass | One next move per role, no table-first landing, no competing primary actions | Employee Suggested Booking Hero, Approver Decision Desk, Reception Handoff Desk and Admin Control Tower are all shipped. Public pre-login surface is now simpler: login/setup plus orientation KPI/calendar/fleet, while inbox and reservation ledger stay authenticated. Deferred: large vanilla module split until after controlled production pilot or until the monolith creates real implementation risk. Rejected for now: framework rewrite, decorative landing visuals, heavy BI, GPS workflow expansion and AI chat. |
...
- `pass`: admin bulk employee import turns a source table into repeatable
  `employee` account maintenance. It uses `Име + Фамилия + GSM`, ignores
  chip/tachograph columns, can reset existing employee passwords and is covered
  by API/UI contract tests.
...
- `pass`: Playwright browser smoke is now split by role and starts a fresh app
  ...
  keyboard recovery.
  Screenshots are written to `test-results/e2e/public-desktop.png`,
  `test-results/e2e/public-mobile.png`,
...
- `evidence`: approver keyboard bulk-selection screenshot
  ...
  timeline checkbox, selected visual state and visible bulk action bar. The
  pure approver first viewport now hides the table fallback.
...
- `evidence`: latest local handoff check ran `make qa-premium` -> dependency
  audit, secret scan, Python compile, 183 pytest cases, JS syntax and 16 Playwright browser
  ...
  succeeded using the current smoke stack. `/ops/readiness` now exposes the
  same restore-drill marker freshness that `make go-live-check` requires, so
  operator UI and shell evidence cannot drift apart silently. PostgreSQL smoke
  is migrated to Alembic revision `20260420_0009`.
