...
| Calendar studio | `templates/index.html`, `static/app.js`, `static/styles.css`, `e2e/test_browser_smoke.py` | pass | Responsive 390/768/1440, keyboard reachability, color+text statuses, role-aware operational visibility, no overlap | Mobile day mode is covered by `employee-mobile.png`; reception role calendar is covered by `reception-desktop.png`. Multi-day records are repeated on each covered date as start/continue/end and range pills are contained in the day cell. Still capture tablet month keyboard day controls. |
...
- `evidence`: latest local handoff check ran `make qa-premium` -> dependency
  audit, Python compile, 149 pytest cases, JS syntax and 12 Playwright browser
