...
| Approved vs active clarity | `static/app.js`, `static/i18n.js`, `static/styles.css`, `templates/index.html`, `tests/test_ui_compliance.py` | pass | Approved-but-not-started must not present as active, fleet cards reflect real operational state | Frontend now derives a dedicated `Чака вземане` presentation state for approved reservations whose start window has begun but reception has not marked `checked_out_at` yet. Current Trip Hero and lifecycle tags use that wording, while fleet cards distinguish `Свободна / Чака вземане / Активен курс / Неактивна` instead of only the pool flag. |
...
| Accepted role-first surface blueprint | `ROADMAP.md`, `ROADMAP_IMPROVEMENTS.md`, `docs/ROLE_USER_FLOWS.md`, future `templates/*.html`, `static/app.js`, `static/styles.css` | partial | One next move per role, no table-first landing, no competing primary actions | Employee Suggested Booking Hero is shipped for free mode and hides once pending/approved/active work exists. Approver Decision Desk is shipped as board-first: cards precede reservations table/filter UI, and bulk selection no longer silently approves. Remaining accepted UI wave: reception Handoff Desk density and admin Control Tower next-focus strip. Deferred: large vanilla module split until after controlled production pilot or until the monolith creates real implementation risk. Rejected for now: framework rewrite, decorative landing visuals, heavy BI, GPS workflow expansion and AI chat. |
...
- `evidence`: latest local handoff check ran `make qa-premium` -> dependency
  audit, secret scan, Python compile, 174 pytest cases, JS syntax and 16 Playwright browser
