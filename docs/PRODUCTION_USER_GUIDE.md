...
- admin започва от Control Tower hero карта с един next move, status badge и
  кратък operational insight list преди настройките.
- одобрен, но непредаден автомобил се вижда като **Чака вземане**, а не като
  **Активен курс**; активен става едва след reception `Започни курс`.
- преди login публичният overview не брои такъв автомобил за **Свободна**, а
  публичните calendar pills показват кратък текстов статус.
- преди login KPI strip-ът показва отделно **Чака вземане**, за да се различи
  approved handoff от реално свободна кола.
- преди login остават само orientation модулите: login/setup, KPI strip,
  календар и флот; inbox и reservations ledger са скрити, за да не се смесва
  публичният вход с вътрешната оперативна работа.
...
За последния manual keyboard/accessibility pass преди go-live използвай
